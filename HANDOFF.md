...
The Radar performance checkpoint moves continuous market indexing into `meerkat-indexer.service`. The web service reads atomic `radar_views` snapshots and SQL counts, so scoring and leaderboard rebuilds no longer block HTTP. Tail cycles rescore only affected tokens and wallets; complete feed and leaderboard snapshots refresh at most every five minutes by default. The metadata queue prioritizes launches with observed activity and processes a configurable batch (production default 40 with concurrency 4); concurrent contract reads share bounded JSON-RPC batches. `MEERKAT_RPC_URLS` accepts an ordered comma-separated provider pool. The terminal labels unprofiled launches `METADATA PENDING`, safely handles empty/non-JSON upstream responses, and exposes a retry action.
