...
interface ObserverOptions {port:number;database:string;market?:MarketReader;discovery?:PonsDiscovery;publicMode?:boolean;publicOrigin?:string;trustedHosts?:string[];historyReader?:ReturnType<typeof historyReader>;indexLimits?:IndexAdmissionOptions}
export async function startObserver(options:ObserverOptions){
 ...
 const address=server.address();if(!address||typeof address==='string')throw new Error('No server address');url=`http://127.0.0.1:${address.port}`;if(!publicMode)monitor.start();
