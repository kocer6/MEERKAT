...
const historyRangeSize=50_000n;
const historyCoverage='Curve trades, native ETH pool swaps, token transfers, factory phases and creator-fee sweeps. Curve buyer/seller addresses are attributed; pool swaps remain unattributed without trace evidence. Events use exact blocks and transactions; per-event timestamps are not fetched from the rate-limited public RPC. Up to cursor only; timeline pages load separately in batches of 50.';
export async function findLaunchBlock(head:bigint,read:(from:bigint,to:bigint)=>Promise<Array<{blockNumber:bigint|null}>>,hint?:bigint){
 if(hint!==undefined&&hint>=0n&&hint<=head){const logs=await read(hint,hint);if(logs.length===1&&logs[0]!.blockNumber===hint)return hint;}
...
export class HistoryStore {
 ...
 eventCount(token:string){return (this.db.prepare('SELECT count(*) AS count FROM token_events WHERE token=?').get(token.toLowerCase()) as {count:number}).count;}
...
export function historyReader(launchHint?:(token:string)=>bigint|undefined){
 const client=createPublicClient({transport:http(process.env.MEERKAT_RPC_URL??'https://rpc.mainnet.chain.robinhood.com',{batch:{batchSize:100,wait:10},timeout:15000,retryCount:1})});
 ...
 async profile(token:string):Promise<TokenProfile>{
  ...
  const lo=await findLaunchBlock(head,(fromBlock,toBlock)=>read(()=>client.getLogs({address:factory,event:launched,args:{token:address},fromBlock,toBlock,strict:true})),launchHint?.(token.toLowerCase()));
  ...
  const c={address,abi,blockNumber:head};
  const [name,symbol,decimals,supply,devBalance,birth,hook,manager,escrow,pending]=await Promise.all([
   read(()=>client.readContract({...c,functionName:'name'})),read(()=>client.readContract({...c,functionName:'symbol'})),read(()=>client.readContract({...c,functionName:'decimals'})),read(()=>client.readContract({...c,functionName:'totalSupply'})),read(()=>client.readContract({...c,functionName:'balanceOf',args:[r.deployer]})),read(()=>client.getBlock({blockNumber:lo})),read(()=>client.readContract({address:factory,abi:parseAbi(['function memeHook() view returns(address)']),functionName:'memeHook',blockNumber:head})),read(()=>client.readContract({address:factory,abi:parseAbi(['function poolManager() view returns(address)']),functionName:'poolManager',blockNumber:head})),read(()=>client.readContract({address:factory,abi:factoryAbi,functionName:'feeEscrow',blockNumber:head})),read(()=>client.readContract({address:factory,abi:factoryAbi,functionName:'pendingCreatorFeeRecipient',args:[address],blockNumber:head})),
  ]);
  const recipientEscrowBalance=await read(()=>client.readContract({address:escrow,abi:parseAbi(['function balanceOf(address recipient) view returns(uint256)']),functionName:'balanceOf',args:[r.creatorFeeRecipient],blockNumber:head}));
...
export class TokenHistory {
 ...
 progress(token:string){token=token.toLowerCase();const state=this.store.state(token),remaining=state?BigInt(state.profile.head)-(state.cursor?BigInt(state.cursor):BigInt(state.profile.birthBlock)-1n):0n;return {state:state??null,running:this.pending.has(token),indexing:{rangeBlocks:Number(historyRangeSize),remainingRanges:remaining>0n?Number((remaining+historyRangeSize-1n)/historyRangeSize):0},events:[] as TokenEvent[],totalEvents:this.store.eventCount(token),wallets:[],score:state?scoreToken(state,[]):null,relationships:null,feeFlow:null,coverage:historyCoverage};}
 ...
 private async run(token:string){let s=this.store.state(token);try{
  ...
  while(from<=head&&!this.stopped){const to=from+historyRangeSize-1n<head?from+historyRangeSize-1n:head;const events=await this.reader.chunk(profile,from,to);s={...s,cursor:to.toString(),status:to===head?'ready':'indexing',updatedAt:Date.now()};this.store.chunk(s,from,to,events);from=to+1n;}
