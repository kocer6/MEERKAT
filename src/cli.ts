...
} else if(['radar-index','radar-collect','radar-enrich','radar-project','radar-metadata'].includes(process.argv[2]??'')){
  const store=new RadarStore(process.env.MEERKAT_DB??'data/observer.sqlite'),indexer=new RadarIndexer(store,radarReader(['radar-enrich','radar-metadata'].includes(process.argv[2]??'')&&process.env.MEERKAT_ENRICH_RPC_URLS?process.env.MEERKAT_ENRICH_RPC_URLS.split(',').map(value=>value.trim()).filter(Boolean):undefined),{...radarOptions(),separateMetadata:true,profileBatchSize:process.argv[2]==='radar-metadata'?40:Number(process.env.MEERKAT_RADAR_PROFILE_BATCH_SIZE??(process.argv[2]==='radar-enrich'?400:40)),profileConcurrency:process.argv[2]==='radar-metadata'?2:Number(process.env.MEERKAT_RADAR_PROFILE_CONCURRENCY??(process.argv[2]==='radar-enrich'?2:4)),marketRead:marketReader(),mode:process.argv[2]==='radar-metadata'?'metadata':process.argv[2]==='radar-collect'?'collect':process.argv[2]==='radar-enrich'?'enrich':process.argv[2]==='radar-project'?'project':'combined',pollMs:['radar-project','radar-metadata'].includes(process.argv[2]??'')?5000:process.argv[2]==='radar-enrich'?30000:radarOptions().pollMs});indexer.start();console.log('MEERKAT Radar indexer started');
  const stop=()=>{void indexer.close().then(()=>{store.close();process.exit(0);});};process.once('SIGINT',stop);process.once('SIGTERM',stop);
} else {
  console.error('Usage: meerkat demo | serve | market | radar-index | radar-collect | radar-enrich | radar-project | radar-metadata');
  process.exitCode = 1;
}
