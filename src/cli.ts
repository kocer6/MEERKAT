...
if (process.argv[2] === 'demo') {
  ...
  const store=new RadarStore(process.env.MEERKAT_DB??'data/observer.sqlite'),indexer=new RadarIndexer(store,radarReader(process.argv[2]==='radar-enrich'&&process.env.MEERKAT_ENRICH_RPC_URLS?process.env.MEERKAT_ENRICH_RPC_URLS.split(',').map(value=>value.trim()).filter(Boolean):undefined),{...radarOptions(),profileBatchSize:Number(process.env.MEERKAT_RADAR_PROFILE_BATCH_SIZE??(process.argv[2]==='radar-enrich'?400:40)),profileConcurrency:Number(process.env.MEERKAT_RADAR_PROFILE_CONCURRENCY??(process.argv[2]==='radar-enrich'?2:4)),marketRead:marketReader(),mode:process.argv[2]==='radar-collect'?'collect':process.argv[2]==='radar-enrich'?'enrich':process.argv[2]==='radar-project'?'project':'combined',pollMs:process.argv[2]==='radar-project'?5000:process.argv[2]==='radar-enrich'?30000:radarOptions().pollMs});indexer.start();console.log('MEERKAT Radar indexer started');
