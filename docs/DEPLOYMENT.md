...
- `meerkat-indexer.service` runs `radar-collect`: factory/market log collection, cursor commits and indexed position accounting. Log requests use a non-batched transport so upstream rate-limit errors remain visible. It makes no metadata or quote calls and does not build leaderboards.
- `meerkat-enrich.service` runs `radar-enrich`: bounded contract profiles (20 missing + up to 5 stale, concurrency 2, 30-second cadence), curve position quotes and public GeckoTerminal/DexScreener batches. Existing profiles and market snapshots become eligible for refresh after five minutes; queue capacity and source availability determine actual freshness. Each field keeps its source/time and missing values remain missing.
...
Set `MEERKAT_ENRICH_RPC_URLS` to isolate contract enrichment from the collector RPC budget. Ordered fallback does not run periodic ranking probes. External market enrichment proceeds even when contract RPC fails. Public RPC quotas remain an operational limit; dedicated endpoints can be supplied through these environment settings without changing application code.
